...
def test_dataset_known_past_data(x1_1d: np.ndarray, y1_1d: np.ndarray) -> None:
    dataset = EncoderDecoderDataset(
        input_data=x1_1d,
        target_data=y1_1d,
...
def test_dataset_known_past_data_too_long(x1_1d: np.ndarray, y1_1d: np.ndarray) -> None:
    with pytest.raises(ValueError):  # noqa: PT011
        EncoderDecoderDataset(
            input_data=x1_1d,
            target_data=y1_1d,
            known_past_data=np.concatenate([x1_1d, x1_1d], axis=-1),
            ctx_seq_len=3,
            tgt_seq_len=2,
        )
