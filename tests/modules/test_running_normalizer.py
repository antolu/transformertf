...
def test_running_normalizer_single_feature() -> None:
    ...
    target = torch.tensor([-1.2649109, -0.6324555,  0.0000000,  0.6324555,  1.2649109])
    ...
    assert torch.allclose(normalizer.get_parameters(), torch.tensor([[3.0, 1.5811390]]))
...
def test_running_normalizer_multi_feature() -> None:
    ...
    # add batch dimension
    x = torch.tensor([[1.0, 2.0], [2.0, 3.0], [3.0, 4.0], [4.0, 5.0]])[None, ...]

    x_transformed = normalizer.fit_transform(x)

    target = torch.tensor([[[-1.1619, -1.1619],
         [-0.3873, -0.3873],
         [ 0.3873,  0.3873],
         [ 1.1619,  1.1619]]])
    ...
    assert torch.allclose(normalizer.scale_, torch.tensor([1.2909945, 1.2909945]))
    assert torch.allclose(normalizer.get_parameters(), torch.tensor([[2.5, 1.2909945], [3.5, 1.2909945]]))
